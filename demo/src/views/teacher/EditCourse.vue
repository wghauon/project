...
const fetchCourseDetail = async () => {
  loading.value = true
  try {
    const res = await courseDetailSearch(courseId)
    if (res.data) {
      const course = res.data.data
      course_name.value = course.course_name || ''
      category_name.value = course.category_name || course.category_id || ''
      ...
      start_time.value = course.start_time || ''
      end_time.value = course.end_time || ''
      join_type.value = String(course.join_type || '1')
      invite_code.value = course.invite_code || ''
      status.value = course.status || 0
      originalCover.value = course.cover_image || ''
      coverImageUrl.value = course.cover_image || ''
    }
  } catch (error) {
...
function buildFormData() {
  ...
  formdata.append('cover_image', originalCover.value)
  return formdata
}
...
async function saveCourse() {
  ...
  try {
    const res = await instance.post('/teacher/course/update', buildFormData())
    if (res.data.status === 0 || res.data.message === '课程信息更新成功') {
      alert('课程修改成功')
...
<template>
  <main class="main-container">
    <div class="page-header">
      <h1 class="page-title">✏️ 编辑课程</h1>
      <p class="page-subtitle">修改课程信息</p>
    </div>
